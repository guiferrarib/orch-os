...
export const CognitionEventUI: React.FC<CognitionEventUIProps> = ({ event, idx, onClick, duration }) => {
  ...
  const getNeuralTypeColor = (type: string | null) => {
    ...
    switch (type.toLowerCase()) {
      ...
      case 'memory': return 'text-orange-400';      // Memória - Laranja (lembranças/recuperação)
      case 'associative': return 'text-amber-500';  // Associativo - Âmbar escuro (conexões entre memórias)
      
      // Cores emocionais/afetivos
      case 'valence': return 'text-amber-400';      // Valência - Âmbar (emoções/afeto)
      case 'soul': return 'text-rose-400';          // Alma - Rosa avermelhado (essência emocional profunda)
      
      // Cores de linguagem/comunicação
      case 'language': return 'text-indigo-400';    // Linguagem - Índigo (expressão verbal)
      
      // Cores de metacognição/planejamento
      case 'metacognitive': return 'text-teal-400'; // Metacognitivo - Turquesa (reflexão sobre pensamento)
      case 'planning': return 'text-sky-400';       // Planejamento - Azul céu (estruturação futura)
      case 'will': return 'text-blue-500';          // Vontade - Azul forte (força de intenção)
      
      // Cores inconscientes/arquetípicos
      case 'unconscious': return 'text-fuchsia-400'; // Inconsciente - Fuchsia (conteúdos não conscientes)
      case 'archetype': return 'text-purple-400';   // Arquétipo - Roxo (padrões universais)
      case 'shadow': return 'text-violet-500';      // Sombra - Violeta escuro (aspectos reprimidos)
      
      // Cores físicos/encarnação
      case 'body': return 'text-green-500';         // Corpo - Verde escuro (sensações físicas)
      
      // Cores sociais/identidade
      case 'social': return 'text-blue-400';        // Social - Azul (interações interpessoais)
      case 'self': return 'text-green-400';         // Self - Verde (identidade/auto-percepção)
      
      // Cores criativos/intuitivos
      case 'creativity': return 'text-pink-400';     // Criatividade - Rosa (inovação/expressão)
      case 'intuition': return 'text-purple-500';   // Intuição - Roxo intenso (conhecimento direto)
      
      // Default para qualquer outro tipo
      default: return 'text-amber-400';             // Default - Âmbar como fallback
    }
  };
  ...
  const getProgressBarColor = (type: string | null) => {
    ...
    switch (type.toLowerCase()) {
      // Cores cognitivos/memória
      case 'memory': return 'bg-orange-500';      // Memória - Laranja 500 (boa visibilidade natural)
      case 'associative': return 'bg-amber-600';   // Associativo - Âmbar 600 (mais intenso para maior contraste)
      
      // Cores emocionais/afetivos
      case 'valence': return 'bg-amber-500';      // Valência - Âmbar 500 (boa visibilidade natural)
      case 'soul': return 'bg-rose-500';          // Alma - Rosa 500 (bom contraste natural)
      
      // Cores de linguagem/comunicação
      case 'language': return 'bg-indigo-600';    // Linguagem - Índigo 600 (intensificado pois índigo tem baixa visibilidade)
      ...
      case 'archetype': return 'bg-purple-600';    // Arquétipo - Roxo 600 (intensificado para garantir visibilidade)
      ...
      case 'self': return 'bg-green-500';          // Self - Verde 500 (verde médio tem boa visibilidade)
      
      // Cores criativos/intuitivos
      case 'creativity': return 'bg-pink-500';     // Criatividade - Rosa 500 (bom contraste natural)
      case 'intuition': return 'bg-purple-600';    // Intuição - Roxo 600 (intensificado para visibilidade)
      
      // Default para qualquer outro tipo
      default: return 'bg-amber-500';             // Default usando âmbar 500 para melhor visibilidade
    }
  };
